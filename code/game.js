...
function Player(pos) {
  ...
  this.size = new Vector(.8, .7);
  this.speed = new Vector(0, 0);
}
Player.prototype.type = "player";

// Add a new actor type as a class
function Coin(pos) { //funky coins???
  var randosize = ((Math.random()*4)+3)*.2;
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(randosize, randosize);
  // Make it GO CIRCUlar motions ish??
  this.wobble = Math.random(10) * Math.PI * 2;
}
Coin.prototype.type = "coin";

//custom sprites?? actors??
function Spike(pos) {
  this.basePos = this.pos = pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(.1, .1);


  this.wobble = Math.random() * Math.PI * 2;
  this.speed = new Vector(0, this.wobble*.5);
  this.repeatPos = pos;
...
// this.wrap corresponds to a div created with class of "game"
  this.wrap = document.getElementById('c1').appendChild(elt("div", "game"));
  this.level = level;
...
Level.prototype.obstacleAt = function(pos, size, speed) {
  ...
  if (yEnd > this.height)
    return "lava";
  if (yEnd >= this.height) //weirdly this fixed some frame issues
      return "spike";
...
//lava movement and behaviors
Lava.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size)) // if not at end?
    this.pos = newPos;
  else if (this.repeatPos) //respawn the lava
...
var maxStep = 0.05;
var playerXSpeed = 9;
var wobbleSpeed = 7, wobbleDist = 0.2;

Coin.prototype.act = function(step) { // new circular motion!!!! for coins??
  this.wobble += step * wobbleSpeed;
  //var wobblePos = Math.sin(this.wobble) * wobbleDist * 3.14;
...
Spike.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  //console.log(level.obstacleAt(newPos, this.size));
  if (!level.obstacleAt(newPos, this.size)) //check for spike modifiers
    this.pos = newPos;
  else if (this.pos <= this.repeatPos.plus(new Vector(0, 1))) //respawn the lava
    this.pos = this.repeatPos;
  else
    this.speed = this.speed.times(-1);
};
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  else  // Move if there's not an obstacle there.
    this.pos = newPos;
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      ...
      if (status == "lost")
        ...
        startLevel(0); //win then reset the game
      }
    });
  }
  startLevel(0);
